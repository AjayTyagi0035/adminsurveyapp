...
import { CircleMarker, TileLayer, Popup, useMap } from 'react-leaflet'
  ...
  const points = useMemo(
    () =>
      mappedRecords.map(record => [
        Number(record.dataLat),
        Number(record.dataLng)
      ]),
    [mappedRecords]
  )
    ...
    >
      ...
      >
        <div>
          ...
          >
            Each marker represents one submission. Click a marker for record
            details.
          </div>
        </div>
                ...
                >
                  <div style={{ minWidth: 180 }}>
                    <div style={{ marginBottom: 6 }}>
                      <strong>
                        {record.ownerName || 'Unnamed owner'}
                      </strong>
                    </div>
                    ...
                    <div>
                      Mohalla: {record.mohallaName || '—'}
                    </div>

                    <div>
                      Ward: {record.wardNoOrName || '—'}
                    </div>
